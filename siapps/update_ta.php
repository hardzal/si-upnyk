...
if($_POST['upload']){
	...
	$vjudul	  = $_POST['judul'];
	$vtmp	  = $_POST['tempat'];
	$xtgl 	  = $_POST['tgldaftar'];
	$ytgl 	  = $_POST['tglstop'];
	$vtglstart= substr($xtgl,6,4).'-'.substr($xtgl,3,2).'-'.substr($xtgl,0,2);
	$vtglstop = substr($ytgl,6,4).'-'.substr($ytgl,3,2).'-'.substr($ytgl,0,2);
	$vpem1    = $_POST['pemb1'];
	$vpem2	  = $_POST['pemb2'];
	$hasil = mysql_query("SELECT * FROM ta WHERE nim='$vnim'");
	if(mysql_num_rows($hasil)>0){ 
		mysql_query("UPDATE  ta SET judul='$vjudul',tempat='$vtmp',tgl_daftar='$vtglstart',tgl_selesai='$vtglstop',
					 pemb_1='$vpem1', pemb_2='$vpem2' WHERE nim='$vnim'"); 
	} else {
		mysql_query("INSERT INTO  ta(nim,judul,tempat,tgl_daftar,tgl_selesai,pemb_1,pemb_2) 
		             VALUES ('$vnim','$vjudul','$vtmp','$vtglstart','$vtglstop','$vpem1','$vpem2')");
	}
}
...
<head>
   <meta charset="utf-8" />
	...
	<script>
		...
		function redir()
		{
		   window.location.href = 'tugas_akhir.php';
		}
	</script>
			...
			<ul class="breadcrumb">
 			...
 			<div class="row-fluid">
				...
				<div class="span6"><br><br>
                  <form class="form-horizontal" action="tugas_akhir.php" method="post" enctype="multipart/form-data">
                    <div class="widget">
                        <div class="widget-title">
                            <h4><i class="icon-reorder"></i> Tugas Akhir</h4>
							<span class="tools">
